<?php
  ...
  class Model{
	...
	public static function mensuelByDate($moisAn){
	 $rep = self::getPdo()->prepare("SELECT nom, prenom, round(AVG(points),2) as moyenne FROM participer p 
	          JOIN epreuve e 	ON p.id_epreuve = e.id JOIN joueur j ON j.id=p.id_joueur 
              WHERE YEAR(e.date)-MONTH(e.date )= :moisAn GROUP BY id_joueur"); 
	 $rep->execute(array('moisAn'=>$moisAn));
	 $rep->setFetchMode(PDO::FETCH_CLASS,'App\models\joueur');    
	 $tab = $rep->fetchAll();
	// var_dump($tab);die();
	return $tab;
	...
	public static function getDate(){
	 ...
	 INNER JOIN epreuve e ON p.id_epreuve =e.id WHERE points IS NOT NULL ");
